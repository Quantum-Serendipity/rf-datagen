...
def test_assemble_parts_multi_domain(tmp_path):
    ...
    iq_nb, tags_nb = assemble_parts(str(tmp_path), window_len=2048, labels=["FT8"])
    ...
    iq_mod, tags_mod = assemble_parts(str(tmp_path), window_len=131072, labels=["BLE"])
...
def test_assemble_parts_validates_shape(tmp_path):
    ...
    iq_data, tags = assemble_parts(str(tmp_path), window_len=2048, labels=["FT8"])
    assert len(tags) == 0
